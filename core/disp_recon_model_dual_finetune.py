...
class RAFTStereoFusion_refine(nn.Module):
    ...
    def forward(self, image1, image2, image1_next, image2_next, exp_h, exp_l, flow_init=None, test_mode=False):
        ...
        image1_next = image1_next.to(device)
        image2_next = image2_next.to(device)
        exp_h = exp_h.to(device)
        exp_l = exp_l.to(device)
        
        # # # #*##### Noise modeling for finetuning#####
        phi_l_exph = ImageFormation(image1, exp_h, device=device)
        phi_r_exph = ImageFormation(image2, exp_h, device=device)
        phi_l_expl = ImageFormation(image1_next, exp_l, device=device)
        phi_r_expl = ImageFormation(image2_next, exp_l, device=device)
        ...
        disable_motion_compensation = getattr(self.args, "disable_motion_compensation", False)
        disable_weighted_fusion = getattr(self.args, "disable_weighted_fusion", False)

        if disable_motion_compensation:
            batch_size, _, height, width = image1.shape
            flow_left = torch.zeros(batch_size, 2, height, width, device=image1.device, dtype=image1.dtype)
            flow_right = torch.zeros(batch_size, 2, height, width, device=image2.device, dtype=image2.dtype)
            warped_fmap_left = fmap1_next
            warped_fmap_right = fmap2_next
        else:
        ...
        #* Resizing mask
        # For logging
        warped_image1_mask = self.warp_feature_map(image1_next_mask, flow_left) if not disable_motion_compensation else image1_next_mask
        #####
        ...
        # Warp mask
        resized_image1_next_mask = self.warp_feature_map(image1_next_mask, flow_left) if not disable_motion_compensation else image1_next_mask
        resized_image2_next_mask = self.warp_feature_map(image2_next_mask, flow_right) if not disable_motion_compensation else image2_next_mask
                
        if disable_weighted_fusion:
            temp_fused_fmap1 = 0.5 * (fmap1 + warped_fmap_left)
            temp_fused_fmap2 = 0.5 * (fmap2 + warped_fmap_right)
            fused_fmap1 = temp_fused_fmap1
            fused_fmap2 = temp_fused_fmap2
            near_zero_mask1 = torch.zeros_like(resized_image1_mask)
            warping_failure_mask1 = torch.zeros_like(resized_image1_mask, dtype=torch.bool)
            update_mask1 = torch.zeros_like(resized_image1_mask)
        else:
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            if disable_motion_compensation:
                warped_outputs08 = cnet_list1_next[0][0]
                warped_outputs16 = cnet_list1_next[1][0]
                warped_outputs32 = cnet_list1_next[2][0]
            else:
            ...
            if disable_weighted_fusion:
                fused_fmap_08 = 0.5 * (cnet_list1[0][0] + warped_outputs08)
                fused_fmap_16 = 0.5 * (cnet_list1[1][0] + warped_outputs16)
                fused_fmap_32 = 0.5 * (cnet_list1[2][0] + warped_outputs32)
            else:
                epsilon = 1e-6
